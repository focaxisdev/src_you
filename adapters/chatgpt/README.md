...
|---|---|
| L0 runtime rules | Custom instructions, project instructions, or an equivalent instruction layer available on the current surface |
| L1 global durable state | A private canonical file or source set that the current surface can reliably read and update |
...
The mapping is capability-based. ChatGPT web, desktop, and Work surfaces do not
expose identical storage or instruction mechanisms. Codex CLI and IDE behavior
is documented separately in [`../codex/`](../codex/). Probe the current
environment before choosing a path.
...
- [Memories](https://learn.chatgpt.com/docs/customization/memories) carry useful
  context forward, while required guidance should remain in durable
  instructions or maintained documentation.
...
- [Personalize ChatGPT](https://learn.chatgpt.com/docs/personalize) documents
  personalization and custom instruction behavior.
...
flowchart TD
    ...
    R --> L["Private maintained file/source state · L1"]
...
2. Use a private maintained file or connected source for L1 when available.
